...
def exampletree():
    return node(ifw,[node(gtw,[paramnode(0),constnode(3)]),node(addw,[paramnode(1)\
            ,constnode(5)]),node(subw,[paramnode(1),constnode(2)]),])
...
def evolve(pc,popsize,rankfunction,maxgen=500,mutationrate=0.1,breedingrate=0.4,pexp=0.7,pnew=0.05):
    ...
    #The lower pexp is, the more lower numbers you will get.
    def selectindex():
...
def gridgame(p):
    ...
    location=[[randint(0,bmax[0]),randint(0,bmax[1])]]
    ...
    for o in range(50):
        ...
        for i in range(2):
            ...
            if move==1:
                location[i][0]+=1
                if location[i][0]>bmax[0]: location[i][0]=bmax[0]
            if move==2:
                location[i][1]-=1
                if location[i][1]<0: location[i][1]=0
            if move==3:
                location[i][1]+=1
                if location[i][1]>bmax[1]: location[i][1]=bmax[1]

            # If you captured the other player you win
...
class humanplayer:
    ...
    def evaluate(self,board):
        ...
        # Return whatevr the user enters
        move=int(raw_input())
        return move